...
df_ = pd.read_csv("C:/Users/Lenovo/PycharmProjects/Armut/armut_data.csv")
df = df_.copy()
df.head()
...
df['New_Date'] = df["CreateDate"].astype('datetime64[M]')
df['New_Date'] = df['New_Date'].dt.to_period('M')

df['CartID'] = df.UserId.astype(str) + "_" + df.New_Date.astype(str)
...
df.dtypes
df_serv_cart = df.groupby(["CartID", "Services"]).agg("CategoryId").count().unstack().applymap(lambda x: 1 if x > 0 else 0)
df_serv_cart.head()
# Sepet-Hizmet matrisi oluşturduk. Sepetlerdeki hizmetleri görebiliyoruz.

# Adım 2: Birliktelik kurallarını oluşturunuz.

df_frequent = apriori(df_serv_cart, min_support=0.01, use_colnames=True)
df_frequent.sort_values("support", ascending=False)
rules = association_rules(df_frequent, metric="support", min_threshold=0.01)
